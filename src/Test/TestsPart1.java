...
public class TestsPart1 {
    ...
    private static void parse_parseDocument_Test1_Dollars() {
        ...
        System.out.println("\ntestThousand");
        ArrayList<String> testThousand = Parse.parseDocument(new Document("0","","123 Thousand 125 thousand"));
        for (int i = 0; i < testThousand.size(); i++) {
            System.out.println(testThousand.get(i));
        }
        System.out.println("\ntestMillionBillionNumber");
        ArrayList<String> testMillionBillionNumber = Parse.parseDocument(new Document("0","","55 Million 60 million 70 Billion 80 billion"));
        for (int i = 0; i < testMillionBillionNumber.size(); i++) {
            System.out.println(testMillionBillionNumber.get(i));
        }
        System.out.println("\ntestPriceDollars");
        ArrayList<String> testPriceDollars = Parse.parseDocument(new Document("0","","1.98 Dollars 1,000,000 Dollars"));
        for (int i = 0; i < testPriceDollars.size(); i++) {
            System.out.println(testPriceDollars.get(i));
        }
        System.out.println("\ntestMillionBillionUSDollars");
        ArrayList<String> testMillionBillionUSDollars = Parse.parseDocument(new Document("0","","100 billion U.S. dollars 100 million U.S. dollars 100 trillion U.S. dollars"));
        for (int i = 0; i < testMillionBillionUSDollars.size(); i++) {
            System.out.println(testMillionBillionUSDollars.get(i));
        }
        System.out.println("\ntestFractions");
        ArrayList<String> testFractions = Parse.parseDocument(new Document("0","","100 3/4 22 1/2 Dollars 112 1/3 dollars 1/2 1/4 Dollars"));
        for (int i = 0; i < testFractions.size(); i++) {
            System.out.println(testFractions.get(i));
        }
        System.out.println("\ntestSimpleNumbers");
        ArrayList<String> testSimpleNumbers = Parse.parseDocument(new Document("0","","98 10,123 1010.56 10,123,000 10,123,000,000"));
        for (int i = 0; i < testSimpleNumbers.size(); i++) {
            System.out.println(testSimpleNumbers.get(i));
        }
        System.out.println("\ntestMonthDate");
        ArrayList<String> testMonthDate = Parse.parseDocument(new Document("0","","June 4 JUNE 4 Aug 24 24 Aug May 1994 October 1996 November"));
        for (int i = 0; i < testMonthDate.size(); i++) {
            System.out.println(testMonthDate.get(i));
        }
        System.out.println("\nbetweenTest");
        ArrayList<String> betweenTest = Parse.parseDocument(new Document("0","","Between 10 and 50 between 70 and 50 between 60 and merav between 40 to 30 between to ty between"));
        for (int i = 0; i < betweenTest.size(); i++) {
            System.out.println(betweenTest.get(i));
        }
        System.out.println("\nsentenceTest");
        ArrayList<String> sentenceTest = Parse.parseDocument(new Document("0","","I am (Merav). I am not amused/invested/interested in this code. I would like to pay (10,000) dollars for someone to replace me!"));
